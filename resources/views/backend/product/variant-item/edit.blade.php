...
@section('content')
	<div class="card shadow-sm">
	    <div class="card-header">
	        <h3 class="card-title">Variant Item Edit</h3>
	    ...
	    <div class="card-body">
	    	...
	    	<form action="{{ route('admin.variantitems.update', ['product' => $product->id, 'variant' => $variant->id, 'variantitem' => $variantitem->id]) }}" method="POST" enctype="multipart/form-data">
	    		@csrf
	    		@method('PUT')
	    		...
	    		<div class="fv-row mb-7">
	    		  ...
	    		  <!--end::Label-->
	    		  <!--begin::Input-->
	    		  <input type="text" name="name" class="form-control form-control-solid mb-3 mb-lg-0 @error('name') is-invalid @enderror" placeholder="Name" value="{{ old('name') ?? $variantitem->name }}" />
	    		  <!--end::Input-->
	    		  @error('name')
	    		...
	    		<div class="fv-row mb-7">
	    		  ...
	    		  <!--end::Label-->
	    		  <!--begin::Input-->
	    		  <input type="text" name="price" class="form-control form-control-solid mb-3 mb-lg-0 @error('price') is-invalid @enderror" placeholder="0.00" value="{{ old('price') ?? $variantitem->price }}" />
	    		  <!--end::Input-->
	    		  @error('price')
	    		      <span class="invalid-feedback" role="alert">
	    		          <strong>{{ $message }}</strong>
	    		...
	    		<div class="fv-row mb-7">
	    		  ...
	    		  <select name="is_default" class="form-select form-select-solid @error('is_default') is-invalid @enderror" data-control="select2" data-placeholder="Select an option" data-allow-clear="true">
	    		      <option></option>
	    		      <option {{ old('is_default', $variantitem->is_default) == "1" ? 'selected' : '' }} value="1">Yes</option>
	    		      <option {{ old('is_default', $variantitem->is_default) == "0" ? 'selected' : '' }} value="0">No</option>
	    		  </select>
	    		  @error('is_default')
	    		      <span class="invalid-feedback" role="alert">
	    		          <strong>{{ $message }}</strong>
	    		...
	    		<div class="fv-row mb-7">
	    		  ...
	    		  <select name="status" class="form-select form-select-solid @error('status') is-invalid @enderror" data-control="select2" data-placeholder="Select an option" data-allow-clear="true">
	    		      <option></option>
	    		      <option {{ old('status', $variantitem->status) == "1" ? 'selected' : '' }} value="1">Active</option>
	    		      <option {{ old('status', $variantitem->status) == "0" ? 'selected' : '' }} value="0">Inactive</option>
	    		  </select>
	    		  @error('status')
	    		      <span class="invalid-feedback" role="alert">
	    		          <strong>{{ $message }}</strong>
