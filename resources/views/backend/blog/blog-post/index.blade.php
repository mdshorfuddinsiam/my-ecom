  ...
  <div class="card">
    ...
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        <a href="{{ route('admin.blogposts.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Blog Post Create</a>
      </div>
      <!--end::Card toolbar-->
    </div>
    <!--end::Card header-->
            ...
            @foreach($blogposts as $row)
              <tr>
                <td>{{ $loop->iteration }}</td>
                  ...
                  <td>
                    <a href="{{ route('admin.blogposts.edit', ['blogpost' => $row->id]) }}" class="btn btn-sm btn-info"><i class="fas fa-edit  "></i></a>
                    <a href="{{ route('admin.blogpost.delete', ['blogpost' => $row->id]) }}" class="btn btn-sm btn-danger delete"><i class="fas fa-trash"></i></a>
                  </td>
              </tr>
              @endforeach
          </tbody>
  ...
  <script>
    ...
    <script>
      $('.status').on('click', function (e) {
             ...
             $.ajax({
                url: "{{ route('admin.blogpost.update-status') }}",
                method: "POST",
                dateType: "JSON",
